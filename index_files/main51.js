...
function ccl(id, from, to, city){
    ...
    if(!city) city = 0;
    last_exch_id = id;
    openDocument('https://www.bestchange.ru/click.php?id=' + id + (from && to ? '&from=' + from + '&to=' + to : '') + '&city=' + city, true);
...
function show_info(id){
    if(typeof direct_data != 'undefined' && direct_data){
        if(id !== show_box){
            ...
            nodeById('det_country').innerHTML = '<img src="https://www.bestchange.ru/images/countries/' + direct_data[id]['c'] + '.svg" valign="middle" />' + direct_data[id]['m'];
        }
    }
}
...
function update_rates(success_callback){
    ...
    if(update_request){
        change_upd_img('', 'https://www.bestchange.ru/images/ajax.png');
...
function check_telegram_link(){
    ...
    telegram_status.innerHTML = '<img src="https://www.bestchange.ru/images/ajax.png" width="16" height="16" alt= />';
...
function update_stats(forcibly){
    ...
    if(forcibly || chartcanvas.className == 'waitchart' || new Date() - chart_time > 2 * 60 * 1000){
        ...
        chartcanvas.innerHTML = '<div class="waitborder"></div> <img src="https://www.bestchange.ru/images/ajax-big.gif" class="waitimg" alt="' + lang['str_ajax_title'] + '" /> <img src="' + chart_url + '" class="chartimg" onload="chart_update_complete(true)" onerror="chart_update_complete(false)" />';
    }
    return false;
}
...
function show_addform(locate, name, u, t){
    ...
    if(show_add_name == 'social'){
        ...
        for(var i = 0; i < social_data.length; i += 3) content += '<a target="_blank" href="http://' + social_data[i] + '/' + social_data[i + 1].replace('{u}', u).replace('{t}', t) + '" title="' + lang['addto_title'] + ' ' + social_data[i + 2] + '"><img src="https://www.bestchange.ru/images/social/' + social_data[i] + '.png" width="16" height="16" alt="' + social_data[i + 2] + '" /></a>';
...
function get_bonus(){
    ...
    if(req){
        ...
        bonus_result.innerHTML = '<img src="https://www.bestchange.ru/images/ajax-big.gif" width="48" height="48" style="padding: ' + (height / 2 - 28) + 'px 0 0 ' + (width / 2 - 24) + 'px" alt="' + lang['str_ajax_title'] + '" />';
        clearTimeout(bonus_captcha_timer);
...
function load_overlay(params){
    ...
    if(req){
        ...
        lightboxContent.innerHTML = '<img src="https://www.bestchange.ru/images/ajax-big.gif" width="48" height="48" style="padding: ' + (height / 2 - 28) + 'px 0 0 ' + (width / 2 - 24) + 'px" alt="' + lang['str_ajax_title'] + '" />';
...
function show_video(id){
    ...
    show_overlay();
    var lightboxContent = document.getElementById('lightboxContent');
    lightboxContent.innerHTML = '<img src="https://www.bestchange.ru/images/ajax-big.gif" id="video_pre_load" width="48" height="48" style="padding: ' + (lightboxContent.offsetHeight / 2 - 28) + 'px 0 0 ' + (lightboxContent.offsetWidth / 2 - 24) + 'px" alt="' + lang['str_ajax_title'] + '" /><div id="video_frame"><iframe onload="setTimeout(function () {video_load_complete()}, 200)" width="600" height="338" src="https://www.youtube.com/embed/' + video + '?rel=0&autoplay=1&showinfo=0" frameborder="0" allowfullscreen></iframe></div>';
}
...
function set_img_status(name, type, noclear){
    if(status_timer != 0) clearTimeout(status_timer);
    status_timer = 0;
    document.getElementById(name + '_img').innerHTML = type ? '<img src="https://www.bestchange.ru/images/' + type + '.png" width="16" height="16" />' : '';
    if(!noclear) status_timer = setTimeout(function(){
